...
var dbConfig = {
  user: "bun_user",
  host: "m4cgyver-archives-backend-postgres",
  database: "webpages",
...
// libs/database/worker.tsx
import net from "net";
var timet = process.hrtime()[1];
var globalDbPool = null;
var server = null;
var port = 9824;
var host = "0.0.0.0";
var parseWarcFilesProgress = {};
var log = (message) => {
  postMessage({ action: "log", message });
};
var setupWorkerDbSocket = () => {
  ...
  server = net.createServer((socket) => {
    ...
    socket.on("data", async (data) => {
      ...
      try {
        ...
        arr.forEach((data2) => handleMessage(data2).then((ret) => socket.write(JSON.stringify(ret))));
      } catch (error) {
...
var dbWorkerInsertResponse = async (params) => {
  const pool = await connectWorkerPool();
  const query = `
  ...
  const values = [
    params.uri_string,
    params.file_string,
    params.content_type_string,
    params.resource_type_string,
    params.record_length,
    params.record_offset,
...
async function dbWorkerSearchResponses(params) {
  const {
    search_uri_a,
    limit_num_a = 32,
    offset_num_a = 0,
...
async function dbWorkerRetrieveResponseFull(uri_string) {
  const client = await connectWorkerPool();
  const query = `
...
var dbWorkerRetrieveLatestResponses = async (total) => {
  const client = await connectWorkerPool();
